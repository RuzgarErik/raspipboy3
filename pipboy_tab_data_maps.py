...
class Mode_Map:
	...
	def __init__(self, *args, **kwargs):
		...
		if (self.mapType == 0):
			...
			self.mapArgs = "&maptype=satellite&style=feature:road|visibility:off&key=AIzaSyBUMK35inQwdUs7ijsJszKTmIZj6_XnSU4"
			self.mapZoom = 18
			...
			self.mapArgs = "&maptype=hybrid&style=feature:road|element:geometry|color:0xDDDDDD&style=element:labels|visibility:off&key=AIzaSyBUMK35inQwdUs7ijsJszKTmIZj6_XnSU4"
			self.mapZoom = 14
	...
	def getMap(self, doDownload = config.USE_INTERNET):
		...
		if doDownload:
			print("DOWNLOADING:")
			mapUrl = "https://maps.googleapis.com/maps/api/staticmap?center=%s" %(self.mapLocation)
